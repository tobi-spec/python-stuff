...
class TestExamples(TestCase):
    ...
    @mock.patch("basic.unittests.unittests.get_list",  return_value=["decorator", "mock"])
    ...
    def test_get_list_mock_context_manager(self):
        with mock.patch("basic.unittests.unittests.get_list", return_value=["manager", "mock"]):
            assert get_list() == ["manager", "mock"]

    def test_get_list_mock_inline(self):
        self.patcher = mock.patch("basic.unittests.unittests.get_list", return_value=["inline", "mock"])
        self.patcher.start()
